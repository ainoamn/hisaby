...
function bhdErrorMessage(bhd: string | null, why: string | null): string | null {
  switch (bhd) {
    case "no_user":
      return "تعذّر إنشاء/ربط مستخدم حسابي بعد الهوية. أعد المحاولة؛ إن استمر راجع سجلات API أو تواصل مع الدعم.";
    case "denied":
    ...
    case "exchange":
      return why && why !== "unknown"
        ? `تعذّر إكمال الدخول بعد الهوية (${why}). أعد المحاولة بعد نشر API الأحدث؛ callback ينشئ المستخدم تلقائياً وفق مواصفة BHD.`
        : "تعذّر إكمال الدخول بعد الهوية. بعد نشر API الأحدث يُنشأ المستخدم تلقائياً (§3.3). أعد المحاولة أو راجع سجلات API.";
    case "error":
      return "تعذّر إكمال الدخول الموحّد. أعد المحاولة، وإن استمر العطل راجع سجلات API.";
    default:
      return null;
  }
}
