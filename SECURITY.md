...
- Cookie mutations still require a matching `X-CSRF-Token`. CSRF origin checks allow `https://hisaby.bhd-om.com`, `FRONTEND_URL`, `CORS_ORIGIN`, and proxy Origins (`*.vercel.app`, `*.onrender.com`, missing Origin). A third-party Origin is still rejected. The SPA reaches Nest through the App Router `/backend-api` proxy, not a Vercel rewrite.
