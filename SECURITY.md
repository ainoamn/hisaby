...
- Cookie mutations require a matching `X-CSRF-Token` when the session is cookie-only. Origin is not used as a CSRF gate: Vercel/Node fetch replace Origin with the API or `*.vercel.app` host, and Render `hisaby-api` has shipped lagging Git commits. The App Router `/backend-api` proxy copies `bhd_access` to `Authorization: Bearer` so the live API (which skips CSRF when that header is present) accepts admin saves. Cross-site POSTs do not send `SameSite=Lax` cookies, so they never receive that Bearer.
